...
lightning::lightning(QGraphicsScene *s,int x,int y):magic(10,6)
{
    for(int i=0;i<=4;i++)
    {
        for(int j=0;j<=tick-1;j++)
        {
            pict[i][j]->setPixmap(QPixmap(":/res/magic/lightning/lightning"+QString::number(j)+".png").scaled(48,48));
            s->addItem(pict[i][j]);
            pict[i][j]->hide();
        }
    }
    ...
    {
        pict[0][i]->setPos(x,y);
        pict[1][i]->setPos(x+48,y+48);
        pict[2][i]->setPos(x-48,y-48);
        pict[3][i]->setPos(x-48,y+48);
        pict[4][i]->setPos(x+48,y-48);
    }
    posx=(x+184)/48;
    posy=(y+154)/48;
}
void lightning::splash()
{
    tick--;
    if(tick==5)
    {
...
void lightning::blast(QLinkedList<unit *> &list)
{
    for(QLinkedList<unit *>::iterator i=list.begin();i!=list.end();i++)
    {
        ...
        if((posx-1==(*i)->posx)&&(posy-1==(*i)->posy))
        {
            (*i)->hp=(*i)->hp-10;
        }
        if((posx+1==(*i)->posx)&&(posy+1==(*i)->posy))
