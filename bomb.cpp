...
bomb::bomb(QGraphicsScene *s,int x,int y):magic(10,6)
{
    for(int i=0;i<=4;i++)
    {
        for(int j=0;j<=tick-1;j++)
        {
        pict[i][j]->setPixmap(QPixmap(":/res/magic/bomb/bomb"+QString::number(j)+".png").scaled(48,48));
        s->addItem(pict[i][j]);
        pict[i][j]->hide();
        }
    }
...
void bomb::blast(QLinkedList<unit *> &list)
{
